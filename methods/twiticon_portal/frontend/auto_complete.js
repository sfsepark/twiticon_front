...
export default (function(tcf){    
    ...
    {
        var curAlias = emote_data.EmoteAutoCompleteTable[curWord];
        ...
        if(curAlias != null)
        {
            ...
            {
                curEmotes.push( emote_data.EmoteInfoList[curAlias[i]['id']] );
            }
            return curEmotes
        }
        else{
    ...
    function appendEmoteRecommendList(curEmotes, x)
    {
        ...
        emoteRecommendDiv.setAttribute('style','position : absolute; left : ' + width + 'px; top :' + (height -87) + 'px;');
    ...
    function replaceEmote(curEmote){
        ...
        var cursor_pos = tcf.chatText.length;
